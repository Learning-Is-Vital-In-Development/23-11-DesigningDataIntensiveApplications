...
- 긍정적 평판
    - 분산 시스템에서 원자적 커밋을 달성하여 안전성 보장을 제공한다.
- 부정적 평판
    - 운영상의 문제를 일으킨다.
    - 성능 이슈의 원인이 된다.
        - MySQL의  분산 트랜잭션은 단일 노드의 트랜잭션보다 10배 이상 느리다.
        - 2PC에서는 장애 복구에 필요한 부가적인 디스크 강제 쓰기(fsync), 부가적인 네트워크 왕복 시간이 추가된다.

분산 트랜잭션은 크게 두 가지 종류로 사용된다.
...
- 이종 분산 트랜잭션
    ...
    - 이종 트랜잭션의 경우 시스템의 내부가 완전히 다르더라도 원자적 커밋을 보장해야 한다.


#### 정확히 한번 메시지 처리

이종 분산 트랜잭션은 다양한 시스템이 강력한 방법으로 통합될 수 있게 한다.

**예시 : 메시지 브로커와 메시지를 처리하는 데이터베이스**

메시지 큐에서 나온 메시지는 그 메시지를 처리하는 DB 트랜잭션이 커밋에 성공했을 떄만 처리된 것으로 확인 받을 수 있다.

- 반대로 메시지 전달이나 데이터베이스 트랜잭션 중 하나가 실패하면 둘 다 어보트되고  메시지 브로커는 나중에 메시지를 안전하게 다시 전달할 수 있다
- 즉, 메시지와 그 처리 과정의 부수 효과를 원자적으로 커밋함으로써 메시지가 **결과적으로(effectively)** 정확히 한 번 처리되도록 보장이 가능하다.


> Appendix. [Apache Kafka exactly-once](https://blog.voidmainvoid.net/504)

**제한사항**

- 위의 분산 트랜잭션은 트랜잭션의 영향을 받는 모든 시스템이 동일한 원자적 커밋 프로토콜을 사용할 수 있을 때만 가능하다.
